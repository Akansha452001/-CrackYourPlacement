...
public:
 bool check(const string& length, const string& str) {
        ...
        for (int i = 0; i < str.length(); i += n) {
            if (str.substr(i, n) != length) return false;
        }
        return true;
    }

    // Function to find the greatest common divisor string of `str1` and `str2`
    string gcdOfStrings(string str1, string str2) {
        string ans;
        int minLength = min(str1.length(), str2.length());
        
        for (int i = 1; i <= minLength; i++) {
            // Only consider substrings that are possible divisors of both strings
            if (str1.length() % i == 0 && str2.length() % i == 0) {
                ...
                if (check(length, str1) && check(length, str2)) {
                    ans = length; // Update the answer with the current valid divisor
                }
            }
        }
        return ans;
